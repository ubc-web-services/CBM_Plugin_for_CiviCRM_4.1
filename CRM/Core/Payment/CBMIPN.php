...
class CRM_Core_Payment_CBMIPN extends CRM_Core_Payment_BaseIPN {
	...
	*/ 
     ...
     * by first checking to see if the transaction is already being processed. If the
     * request has not been processed already, it completes the transaction and then
     * and finaly sends a notification response back to the CBM Server.  
     * 
     * This function calls the processor non-specific functions getContext() and newOrderNotify()
	 * 
    ...
    static function processNotificationResponse($sessionAuthTicket, $xmlFieldArray, $cbm_trxn_auth_url, $cbm_trxn_auth_port, $cbm_trxn_state) {
		...
		//Validate the incoming session ticket with CBM
	  	$sessionResponseArray =  self::validateCBMAuthTicket($cbm_trxn_auth_url, $cbm_trxn_auth_port, $sessionAuthTicket);
	
	  	if ($sessionResponseArray['response-code'] == "100") { //Code 100 == valid ticket
	  		$newState = "COMPLETE";
        ...
        $privateData = array();
        $privateData['contactID'] = $values['contact_id'];
		$privateData['contributionID'] = $values['contribution_id'];
		$privateData['contributionTypeID'] = $values['contributionType_id'];
		$privateData['invoiceID'] = $values['invoice_id'];

		$component = $Trxn_Obj->component;
		if ($component == "event") {
			$privateData['participantID'] = $values['participant_id'];
			$privateData['eventID'] = $values['event_id'];
		} 
		elseif ($component == "contribute") {
	        $privateData["membershipID"] = $values['membership_id'];			
		}
		...
		if ($duplicateTransaction == 0) {
			$ipn->newOrderNotify(1, $privateData, $component, $values['amount'], $values['auth_ticket']);
		}
        ...
        if ($transaction_status == '0') {
			if ($component == "event") {
                $finalURL = CRM_Utils_System::url( 'civicrm/event/register',
                                                   "_qf_ThankYou_display=1&qfKey=".$values['qfkey'], 
                                                   false, null, false );
			} 
			elseif ($component == "contribute") {
		    ...
		    if ($component == "event") {
                $finalURL = CRM_Utils_System::url( 'civicrm/event/confirm',
                                                   "reset=1&cc=fail&participantId=".$values['participant_id'],
                                                   false, null, false );
            } 
            elseif ($component == "contribute") {
                $finalURL = CRM_Utils_System::url( 'civicrm/contribute/transact',
                                                   "_qf_Main_display=1&cancel=1&qfKey=".$values['qfkey'],
                                                   false, null, false );
            }
